...
class Credits:
    def __init__(self, screen):
        ...
        self.fond = pygame.transform.scale(
            fond, (self.x, self.y)
        )  # Mise à l'échelle de l'image de fond
        self.surface = pygame.Surface(
            (SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA
        )  # Création d'une surface transparente pour le texte
        self.screen = screen
    ...
    def afficher_texte(self):
        ...
        for ligne in self.texte:
            texte_surface = self.police.render(
                ligne, True, self.noir
            )  # Rendu du texte avec la police et la couleur
            texte_rect = texte_surface.get_rect(
                center=(self.x + 250, y_pos)
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        AudioManager().play_bgm(
                            "main_menu", loop=-1, introName="main_menu_intro"
                        )
                        running = False

            self.y -= self.vitesse_scroll  # Déplacement vertical du texte vers le haut
            if (
                self.y < -len(self.texte) * 30
            ):  # Arrêt du défilement une fois que tous les crédits sont passés
