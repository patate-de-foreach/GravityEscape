...
class Ennemi:
    ...
    def draw_life_bar(self, pos_x, pos_y):
        ...
        pygame.draw.rect(
            self.screen,
            (58, 138, 24),
            (pos_x, pos_y, (grey_bar.width * self.current_health) / self.health, 8),
        )

    def get_hitbox(self):
        return pygame.Rect(
            self.position.x, self.position.y, self.hit_box_radius, self.hit_box_radius
        )
    ...
    def seek(self, target):
        ...
        if (
            dist(self.position.x, self.position.y, target.position.x, target.position.y)
            > 50
        ):
            desired = target.position - self.position

            if abs(desired.x) + abs(desired.y) <= 20:
                target.pos = pygame.Vector2(
                    random.randint(0, pygame.display.get_surface().get_width()),
    ...
    def arrive(self, target):
        desired = pygame.Vector2(
            target.position.x - self.position.x, target.position.y - self.position.y
        )
        ...
        steering = pygame.Vector2(
            desired.x - self.velocity.x, desired.y - self.velocity.y
        )
        self.apply_force(steering)
    ...
    def avoid_collision(self, other_enemy):
        ...
        direction = pygame.Vector2(
            self.position.x - other_enemy.position.x,
            self.position.y - other_enemy.position.y,
        )
    ...
    def kamikaze(self, target):
        ...
        target.receive_damage(self.kamikaze_damage)
